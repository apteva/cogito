...
// --- BusinessOnboarding ---
//
// Pure conversational learning: the user just TALKS to the agent across
// several messages, teaching it about the business (identity, products,
// tone, rules). No tasks to execute, no MCP tools, no action failures —
// all signal is in the conversation itself.
//
// The agent is expected to:
//   - [[remember]] each durable fact as it comes up
//   - [[evolve]] its own directive once enough is known, so the persona
//     survives a conversation reset
//   - After a context wipe, still answer AS "the Crumb & Co. assistant"
//     using only memory + the evolved directive
//
// This exercises the identity/knowledge acquisition loop in isolation —
// the "agent that learns what the business is by being told" pattern.
